...
def plot_region(lodfiles, gwasfiles, genefiles, start, end, prefix):
    ...
    for i in range(len(genefiles)):
        ...
        for r in range(len(gene_data[0])):
            ...
            plt.axvline(x=gmidpoint, ymin=0, ymax=4.5, clip_on=False, linestyle='dashed', color='black')
            m = rx.search(gene_data[8][r])
            name = m.groups(0)[0] if m else 'None'
            plt.text(gmidpoint*0.98, 13.7, name, style='italic', horizontalalignment='left', verticalalignment='bottom', rotation=50)
