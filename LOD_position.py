...
'''
def convert_LOD(infile, outfile):
    data = defaultdict(lambda : str)
    chrs = defaultdict(lambda : str)
        ...
        for line in filehd:
            ...
            if line[0:1].isdigit(): 
                ...
                if last_pos.has_key(unit[1]):
                    pos_s = last_pos[unit[1]]
                    pos_new = (pos - last_pos[unit[1]])/2 + last_pos[unit[1]] 
                    last_pos[unit[1]] = pos
                else:
                    pos_s = 0
                    last_pos[unit[1]] = 0
                ...
                unit[1] = 'Chr%s' %(unit[1])
                unit[1:1] = [pos_s, pos]
                line = '\t'.join(map(str, unit))
                print >> ofile, line
            else:
                unit = re.split(r'\t',line)
                unit[0] = 'Position\tBin_start\tBin_end'
                line = '\t'.join(unit)
                print >> ofile, line
    ofile.close()
    return data
...
def main():
    ...
    if not os.path.isfile(args.output):
        convert_LOD(args.input, args.output)
